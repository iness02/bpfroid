...
#define ALERT_WRITE_FORBIDDEN 5
...
int BPF_KPROBE(trace_security_file_permission_entry)
{
    ...
    if (!event_chosen(WRITE_FORBIDDEN_ALERT))
        return 0;
    return save_args_from_regs(ctx, WRITE_FORBIDDEN_ALERT, false);
}

SEC("kretprobe/security_file_permission")
int BPF_KPROBE(trace_ret_security_file_permission)
{
    args_t args = {};
    bool delete_args = true;
    if (load_args(&args, delete_args, WRITE_FORBIDDEN_ALERT) != 0)
        return 0;
    ...
    // Only alert on write permission checks (MAY_WRITE = 0x2)
    int mask = (int)args.args[1];
    if (!(mask & MAY_WRITE))
        return 0;

    // Only alert when permission was denied (return value < 0)
